...
/// Wrapper around pond::Volume that operates on paths, rather than Ino and names.
pub(crate) struct PathVolume {
    inner: pond::Volume,
}
...
async fn resolve_fileattr(volume: &pond::Volume, path: impl AsRef<str>) -> pond::Result<&FileAttr> {
...
async fn resolve_parent_ino_and_filename(
    ...
    let parent = parent(path.as_ref()).ok_or(pond::ErrorKind::InvalidData)?;
...
impl PathVolume {
    ...
    pub(crate) async fn commit(&mut self, version: impl AsRef<str>) -> pond::Result<()> {
        let version = Version::from_str(version.as_ref())?;
        self.inner.commit(version).await
    }

    /// Fetch metadata of a file if it exists.
    pub(crate) async fn metadata(&self, path: impl AsRef<str>) -> pond::Result<FileAttr> {
        let attr = resolve_fileattr(&self.inner, &path).await?;
        Ok(attr.clone())
    }

    /// Check if a file or directory exists.
    pub(crate) async fn exists(&self, path: impl AsRef<str>) -> pond::Result<bool> {
    ...
    pub(crate) async fn read_dir(&self, path: impl AsRef<str>) -> pond::Result<()> {
        let attr = resolve_fileattr(&self.inner, &path).await?;
        match attr.kind {
    ...
    pub(crate) async fn create_dir(&mut self, path: impl AsRef<str>) -> pond::Result<FileAttr> {
        let (parent_ino, name) = resolve_parent_ino_and_filename(&self.inner, path).await?;
        self.inner.mkdir(parent_ino, name).cloned()
    }

    /// Create a directory in the volume.
    pub(crate) async fn create_dir_all(&mut self, path: impl AsRef<str>) -> pond::Result<FileAttr> {
        todo!()
    }

    /// Remove a directory from the volume.
    pub(crate) async fn remove_dir(&mut self, path: impl AsRef<str>) -> pond::Result<()> {
        let (parent_ino, name) = resolve_parent_ino_and_filename(&self.inner, path).await?;
        self.inner.rmdir(parent_ino, &name)
    }

    pub(crate) async fn remove_dir_all(&mut self, path: impl AsRef<str>) -> pond::Result<()> {
        todo!()
    }

    pub(crate) async fn open(&mut self, path: impl AsRef<str>) -> pond::Result<()> {
        todo!()
    }

    /// Remove a file from the volume.
    pub(crate) async fn remove_file(&mut self, path: impl AsRef<str>) -> pond::Result<()> {
    ...
    pub(crate) async fn touch(&mut self, path: impl AsRef<str>) -> pond::Result<FileAttr> {
        let (parent_ino, name) = resolve_parent_ino_and_filename(&self.inner, path).await?;
